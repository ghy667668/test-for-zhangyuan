...
# 新增：为Gnuplot生成数据文件
print("\n正在为Gnuplot生成轨迹数据文件...")
gnuplot_data_dir = "gnuplot_data"
if not os.path.exists(gnuplot_data_dir):
    os.makedirs(gnuplot_data_dir)

for i, trajectory in enumerate(sample_data['trajectories']):
    filename = os.path.join(gnuplot_data_dir, f"trajectory_{i+1}.dat")
    with open(filename, 'w', encoding='utf-8') as f:
        f.write("# 政府监管强度(X) 机构遵从度(Y) 公众监督强度(Z)\n")
        for point in trajectory:
            f.write(f"{point[0]:.6f} {point[1]:.6f} {point[2]:.6f}\n")
print(f"  已保存: {filename}")


# 1. Set graphical output (uncomment the following two lines if you want to save the image)
# set terminal pngcairo size 1200,800 enhanced font ',12'
# set output 'evolution_3d_final.png'

# 2. Use interactive terminal to display on screen first
set terminal qt enhanced
set title 'Three-Party Evolutionary Game: 3D Trajectories of Elderly Care Service Quality Supervision' font ',16'
set xlabel 'Government Supervision Intensity (X)' font ',12'
set ylabel 'Institution Compliance (Y)' font ',12'
set zlabel 'Public Supervision Intensity (Z)' font ',12'
set grid front lt 0 lw 1 lc rgb '#D0D0D0'

# 3. Set an improved 3D viewing angle (adjust these two angles to find the best view)
#    elev: elevation (looking up/down), azim: azimuth (rotating left/right)
set view 68, 32, 1.2, 1.0

# 4. Set the position and style of the legend (key)
set key outside right top vertical Left reverse enhanced
set key box lt 1 lw 1
set key spacing 1.5 font ',11'
set key maxrows 6

# 5. Define line styles and colors for the 5 trajectories
set style line 1 lc rgb '#1f77b4' lw 3 pt 7 ps 0.5 dt 1 # Blue, solid line
set style line 2 lc rgb '#ff7f0e' lw 3 pt 7 ps 0.5 dt 1 # Orange, solid line
set style line 3 lc rgb '#2ca02c' lw 3 pt 7 ps 0.5 dt 1 # Green, solid line
set style line 4 lc rgb '#d62728' lw 3 pt 7 ps 0.5 dt 1 # Red, solid line
set style line 5 lc rgb '#9467bd' lw 3 pt 7 ps 0.5 dt 1 # Purple, solid line

# 6. Plot all 5 trajectories
splot \
    'gnuplot_data/trajectory_1.dat' u 1:2:3 w lines ls 1 t 'Case 1: All Parties Passive', \
    'gnuplot_data/trajectory_2.dat' u 1:2:3 w lines ls 2 t 'Case 2: Government Active', \
    'gnuplot_data/trajectory_3.dat' u 1:2:3 w lines ls 3 t 'Case 3: Institution High Quality', \
    'gnuplot_data/trajectory_4.dat' u 1:2:3 w lines ls 4 t 'Case 4: Public Active', \
    'gnuplot_data/trajectory_5.dat' u 1:2:3 w lines ls 5 t 'Case 5: All Parties Active'

# 7. Keep the graph window open
pause mouse "Click on graph window or press any key to exit."

Save the above Gnuplot script as plot_trajectories.gp and place it in the same directory as the gnuplot_data folder.

Open Gnuplot, switch to that directory (use the command cd "your directory path" in Gnuplot), and then type: load 'plot_trajectories.gp'

You will get a high-quality 3D trajectory plot named evolution_trajectories_3d.png.
